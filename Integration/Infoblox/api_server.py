...
class ApiServer:
    ...
    def forward_request_to_fs(
        ...
        # getData() resolves the API server credentials + endpoint via
        # TableParams(param, root_schema='Infoblox'), which looks up the API
        # server document by domain_db_name + apiServerId. getData reads these
        # from the param it receives (i.e. from api_params), so they MUST live
        # INSIDE api_params -- not as siblings of it. Left at the sibling level
        # they never reach getData, TableParams returns None, and it crashes on
        # extra_params.get('user').
        if isinstance(api_params, dict):
            api_params = dict(api_params)
            api_params["domain_db_name"] = self.domain_name
            api_params["apiServerId"] = self.id
        ...
        # func_args carries exactly what getData receives as its `param`:
        # {"url", "api_parm", "domain_db_name", "apiServerId"} -- mirroring the
        # reference getData(rtn_params) call, plus the credential-lookup keys.
        tech_param["func_args"] = json.dumps(api_params)

        pluginfw.AddLog(
            f"Function [{func_name}] requested with api_params: "
            f"{json.dumps(api_params)}",
            pluginfw.INFO,
        )
        ...
        pluginfw.AddLog(
            f"Function [{func_name}] raw response "
            f"(type={type(response).__name__}, len={response_len}): {response}",
            pluginfw.INFO,
        )
