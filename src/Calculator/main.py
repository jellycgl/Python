...
class Calculator(object):
    ...
    def menus(self):
        ...
        filemenu.add_command(label='Standard(T)     Alt+1', command=self.nofunc)
        filemenu.add_separator()
        filemenu.add_command(label='Scientific(S)   Alt+2', command=self.nofunc)
        filemenu.add_command(label='Programmer(P)   Alt+3', command=self.nofunc)
        allmenu.add_cascade(label='View(V)', menu=filemenu)
        self.root.config(menu=allmenu)

    def layout(self):
        result = tkinter.StringVar()
        result.set(0)
        show_label = tkinter.Label(self.root, bd=3, bg='white', font=('Helvetica', 30), anchor='e', textvariable=self.result)
    ...
    def nofunc(self):
        tkinter.messagebox.showinfo('', 'This function has not yet been implemented!')
    ...
    def wait(self):
        tkinter.messagebox.showinfo('', 'Updating......')
